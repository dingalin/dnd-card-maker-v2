...
export interface SafeAreaDetectionOptions {
    startX?: number | null;
    startY?: number | null;
...
export class SafeAreaDetector {
     ...
     */
    static detect(source: HTMLImageElement | HTMLCanvasElement, options: SafeAreaDetectionOptions = {}): SafeAreaBox {
        const {
        ...
        const queue: Point[] = [{ x: centerX, y: centerY }];
     ...
     */
    private static _getAverageColor(data: Uint8ClampedArray, width: number, height: number, centerX: number, centerY: number, size: number): Color {
     ...
     */
    private static _colorDistance(r1: number, g1: number, b1: number, r2: number, g2: number, b2: number): number {
        return Math.sqrt(
            Math.pow(r1 - r2, 2) +
            Math.pow(g1 - g2, 2) +
            Math.pow(b1 - b2, 2)
     ...
     */
    static visualize(source: HTMLImageElement | HTMLCanvasElement, safeArea: SafeAreaBox): HTMLCanvasElement {
        const canvas = document.createElement('canvas');
        let width: number, height: number;
